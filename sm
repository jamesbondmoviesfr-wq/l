...
local SaveManager = {} do
    ...
    function SaveManager:BuildConfigSection(tab)
        ...
        section:AddButton("Create Config", function()
            name = self.Library.Options.SaveManager_ConfigName.Value

            if name:gsub(" ", "") == "" then
                return self.Library:Notify("Invalid config name", 2)
            end

            success, err = self:Save(name)
            if not success then
                return self.Library:Notify("Failed to create: " .. err)
            end

            self.Library:Notify("Created: " .. name)
        ...
        self.Library.Options.SaveManager_ConfigList:OnChanged(function()
            self.Library.Options.SaveManager_ConfigList:SetValues(self:RefreshConfigList())
        end)
        ...
        section:AddButton("Save Config", function()
            ...
            self.Library:Notify("Saved: " .. name)
        end)
        ...
        section:AddButton("Load Config", function()
            ...
            self.Library:Notify("Loaded: " .. name)
        end)

        section:AddButton("Delete Config", function()
            name = self.Library.Options.SaveManager_ConfigList.Value

            success, err = self:Delete(name)
            if not success then
                return self.Library:Notify("Failed to delete: " .. err)
            end
        ...
        section:AddButton("Set AutoLoad", function()
            ...
            success, err = self:SaveAutoloadConfig(name)
            if not success then
                return self.Library:Notify("Failed to set autoload: " .. err)
            end

            self.Library:Notify("Set autoload: " .. name)
        end)

        section:AddButton("Remove AutoLoad", function()
            success, err = self:DeleteAutoLoadConfig()
            if not success then
